...
启动后，访问 [http://localhost:8888](http://localhost:8888) 即可进入全新的中文管理后台。**首次访问将会自动跳转到安装页面，请按照提示创建管理员账号。**
...
- **系统设置**: 开启/关闭局域网发现 (mDNS) 广播，设置服务名称。

### 局域网发现 (mDNS)
Mochi MQTT 支持通过 mDNS (Multicast DNS) 在局域网内广播服务，方便设备自动发现服务器 IP 和端口。
- **默认状态**: 关闭 (False)
- **开启方式**:
    1. **环境变量**: 在 `.env` 文件中设置 `MDNS_ENABLED=true` 和 `MDNS_NAME=MyBroker`。
    2. **管理后台**: 进入“系统设置”页面进行动态配置（无需重启）。
- **广播服务**: `_mqtt._tcp`
